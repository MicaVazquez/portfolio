          ...
          <div key={group.name} className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2.5">
              {group.items.map((skill) => (
                <span
                  key={skill.name}
                  className="inline-flex items-center gap-2.5 text-sm px-[15px] py-2.5 rounded-[10px] bg-brand-surface/40 backdrop-blur-md border border-brand-highlight/22 text-brand-highlight hover:border-brand-highlight hover:text-brand-cream transition-colors"
                >
                  <skill.icon />
                  {skill.name}
                </span>
